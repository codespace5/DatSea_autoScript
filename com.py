...
def process_files(file_path, output_folder, start_row, end_row, log_file):
    ...
    with open(log_file, 'a') as log:
        for index, row in selected_rows.iterrows():
            file_path = row[2]  # Assuming the third column is at index 2
            ...
            try:
                if file_path.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
                    ...
                    print(f"Saved image {file_name} to {output_path}")
                elif file_path.lower().endswith('.json'):
                    shutil.copy(file_path, output_path)
                    print(f"Copied JSON file {file_name} to {output_path}")
...
def start_processing():
    ...
    process_files(file_path, output_folder, start_row, end_row, log_file)
    messagebox.showinfo("Success", "Files processing completed.")
